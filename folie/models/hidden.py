...
class EncapsulateHidden(ParametricFunction):
    """
    ...
    def grad_x(self, x, *args, **kwargs):
        return self.f.grad_x(x) + np.einsum("t...he,th-> t...e", self.g.grad_x(x), x[:, self.dim_x :])

    def grad_coeffs(self, x, *args, **kwargs):
        """
        Jacobian of the force with respect to coefficients
        """
        return np.concatenate((self.f.grad_coeffs(x[:, : self.dim_x]), np.einsum("t...hc,th-> t...c", self.g.grad_coeffs(x[:, : self.dim_x]), x[:, self.dim_x :])), axis=-1)
...
class OverdampedHidden(Overdamped):
    ...
    def __init__(self, force, friction, diffusion, dim=1, dim_h=0, **kwargs):
        ...
        self.meandispl = EncapsulateHidden(self.force, self.friction, self.dim_x)
        # TODO: Encapsuler aussi la diffusion
