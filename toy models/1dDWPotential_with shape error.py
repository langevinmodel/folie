...
model_simu = fl.models.Overdamped(force_function,diffusion=diff_function)
simulator = fl.simulations.Simulator(fl.simulations.EulerStepper(model_simu), 1e-3)#, k=0.0, xstop=6.0)
...
xp = np.asarray(xin).reshape(-1, 1)
print(xp.shape[0])
print("shape=",xin.shape)
# Calculate Trajectory
time_steps=1000
data = simulator.run(time_steps, xin,save_every=1)
